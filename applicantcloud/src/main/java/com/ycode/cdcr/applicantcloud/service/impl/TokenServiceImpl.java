...
public class TokenServiceImpl implements TokenService {
  ...
  private static final String LOGIN_USER_KEY = "LOGIN_SXENT_USER_KEY";
  private static Key KEY = null;
  private static final String REDIS_KEY_PRO = "CDCR_TOKEN:token-";
  ...
  @Override
  public Token save(LoginApUserVo loginApUserVo) {

    String uuid = UUID.randomUUID().toString();
    String jwtToken = createJWTToken(uuid);

    //登入信息存redis
    myRedisOperate.set(REDIS_KEY_PRO + uuid, JSON.toJSONString(loginApUserVo), expireSeconds);

    return new Token(jwtToken, System.currentTimeMillis());
  }

  @Override
  public LoginApUserVo getLoginUser(String jwtToken) {
    String uuid = getUUIDFromJWT(jwtToken);
    if (uuid != null) {
      String Sign = REDIS_KEY_PRO + uuid;
      if (myRedisOperate.keyExists(Sign)) {
        LoginApUserVo loginUser = FastJsonUtil.toBeanFromStr(myRedisOperate.get(Sign),
            LoginApUserVo.class);
        return loginUser;
      } else {
        return null;
