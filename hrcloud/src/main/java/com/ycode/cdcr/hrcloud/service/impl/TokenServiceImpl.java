...
public class TokenServiceImpl implements TokenService {
  ...
  @Override
  public Token save(LoginHrVo loginHrVo) {

    String uuid = UUID.randomUUID().toString();
    String jwtToken = createJWTToken(uuid);

    //登入信息存redis
    myRedisOperate.set(REDIS_KEY_PRO + uuid, JSON.toJSONString(loginHrVo), expireSeconds);

    return new Token(jwtToken, System.currentTimeMillis());
  }

  @Override
  public LoginHrVo getLoginUser(String jwtToken) {
    String uuid = getUUIDFromJWT(jwtToken);
    if (uuid != null) {
      String Sign = REDIS_KEY_PRO + uuid;
      if (myRedisOperate.keyExists(Sign)) {
        LoginHrVo loginHrVo = FastJsonUtil.toBeanFromStr(myRedisOperate.get(Sign),
            LoginHrVo.class);
        return loginHrVo;
      } else {
        return null;
